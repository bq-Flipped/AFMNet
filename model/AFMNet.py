...
class Encoder(nn.Layer):
    def __init__(self, in_channels: int, out_channels: int, block_num: int, stage_num: int = 2,
        ...
        self.blocks = nn.Sequential(*[
            AFM(out_channels, drop_path_rate=drop_path, kernel_sizes=kernel_sizes)
...
class HSGFM(nn.Layer):
    def __init__(self, feat_channels: Sequence[int]):
        super(HSGFM, self).__init__()
...
class AFMNet(nn.Layer):
    def __init__(
        ...
        self.stage1 = Encoder(feat_channels[0], feat_channels[0], block_num[0], stage_num=1, drop_path=dropout_rate,
                           kernel_sizes=(3, 7))  
        self.stage2 = Encoder(feat_channels[0], feat_channels[1], block_num[1], drop_path=dropout_rate,
                           kernel_sizes=(3, 7))  
        self.stage3 = Encoder(feat_channels[1], feat_channels[2], block_num[2], drop_path=dropout_rate,
                           kernel_sizes=(5, 9))

        self.decoder = HSGFM(feat_channels)
...
def AFMNet_B(num_classes=2):
    model = AFMNet(32, (1, 1, 2), (32, 64, 128), dropout_rate=0.1, pretrained=None)
    return model

@manager.MODELS.add_component
def AFMNet_M(num_classes=2):
    model = AFMNet(32, (2, 2, 3), (32, 64, 128), dropout_rate=0.1, pretrained=None)
    return model

@manager.MODELS.add_component
def AFMNet_L(num_classes=2):
    model = AFMNet(32, (3, 3, 4), (32, 64, 128), dropout_rate=0.1, pretrained=None)
    return model

if __name__ == "__main__":
    model = AFMNet_B(num_classes=2)
